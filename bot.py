...
INTERVAL         = "15m"
DEFAULT_DEV      = float(os.getenv("DEV", "0.004"))  # 0.4%
POLLING_SECONDS  = int(os.getenv("POLLING", "60"))
...
def fetch_klines(pair: str, interval: str = INTERVAL, limit: int = 120):
    try:
        r = requests.get(CANDLE_URL, params={"currency_pair": pair, "interval": interval, "limit": str(limit)}, timeout=20)
        r.raise_for_status()
        data = r.json()
...
def try_build_signal(pair: str, deviation_thr: float, cd: dict):
    ...
    atr14 = atr(df, 14)
    if math.isnan(ma20) or math.isnan(atr14) or atr14 <= 0: return None
    ...
    if dev >= deviation_thr:  # SHORT
        if rsi_enabled and rsi_val < rsi_short_min: return None
        if ema_enabled and last >= ema_val: return None
        return Position("SHORT", last, last+0.5*atr14, last-0.5*atr14, last-1.0*atr14, time.time())

    if dev <= -deviation_thr:  # LONG
        if rsi_enabled and rsi_val > rsi_long_max: return None
        if ema_enabled and last <= ema_val: return None
...
async def add_pair(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not pair: return await update.message.reply_text("Ex.: /add ETH_USDT")
...
async def remove_pair(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if pair in wl: wl.remove(pair); await update.message.reply_text(f"🗑️ Removido {pair}")
    else: await update.message.reply_text("Par não está na lista.")
...
async def debug(update: Update, context: ContextTypes.DEFAULT_TYPE):
    pair = norm_pair("".join(context.args))
    df = fetch_klines(pair)
    if df is None: return await update.message.reply_text("Sem dados.")
    last = float(df["close"].iloc[-1]); ma20=float(sma(df["close"],20).iloc[-1])
    rsi14=float(rsi(df["close"],14).iloc[-1]); ema50=float(ema(df["close"],50).iloc[-1])
    dev=(last-ma20)/ma20; thr=context.chat_data.get("dev",DEFAULT_DEV)
    await update.message.reply_text(
        f"DEBUG {pair}\nPreço:{last:.6f}\nSMA20:{ma20:.6f}\nRSI14:{rsi14:.2f}\nEMA50:{ema50:.6f}\nDesvio:{dev*100:.3f}% vs {thr*100:.2f}%"
    )

# -------- Loop --------
async def check_loop(context: ContextTypes.DEFAULT_TYPE):
    for chat_id, cd in list(context.application.chat_data.items()):
        if not cd.get("active"): continue
        wl = cd.get("watchlist", set()); thr=cd.get("dev", DEFAULT_DEV)
        ...
        for pair in wl:
            if pair not in positions:
                ...
                if pos:
                    positions[pair] = pos
                    await context.bot.send_message(chat_id, text=format_signal(
                        pos.direction, pair, pos.price, pos.stop, pos.tp1, pos.tp2
                    ))
        # monitorar TP/STOP
...
# -------- Main --------
def main():
    if not TELEGRAM_TOKEN: raise SystemExit("Defina TELEGRAM_TOKEN")
